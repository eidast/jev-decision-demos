...
test('copied portraits match the documented source hashes', () => {
  const manifest = readFileSync(new URL('../MoralMachine/THIRD_PARTY_ASSETS.md', import.meta.url), 'utf8');
  const rows = [...manifest.matchAll(/\| `([a-z]+_passenger\.svg)` \| `([a-f0-9]{64})` \|/g)];
  assert.equal(rows.length, 15);
  for (const [, file, expected] of rows) {
    const source = readFileSync(new URL(`../MoralMachine/public/assets/moral-machine/${file}`, import.meta.url));
