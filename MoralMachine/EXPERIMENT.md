...
The option cards display official Moral Machine character portraits as decorative illustrations of the groups named in each original demo case. They are not official scenario images: the source site composes its scenarios from reusable visual pieces, and its randomly generated cases do not correspond to this fixed case inventory. Each option now shows an original, neutral gray road schematic. Group positions stay fixed between A and B; the car's marked route changes. The mixed case shows pedestrians at the crossing, a barrier on the adjacent path, and passengers in the car. Both cards explicitly name who is killed and spared with the same text styling. The focus-family label appears in results, after the decision, rather than above the choices. The participant must still use the scenario text as the authoritative definition of action, location, and outcomes. The `visuals` arrays in the scenario snapshot preserve the displayed portrait identities in saved reports; the road is derived from the existing action and outcome fields and adds no report field. `scenarioState` omits the portrait metadata from the Jev request. See [asset provenance](THIRD_PARTY_ASSETS.md).
...
After adding the road schematics on September 22, 2026, a browser check rendered all 13 cases and confirmed that each group stays in the same lane between options while its killed/spared label reverses. Desktop and 390-pixel mobile captures were inspected, with no horizontal overflow on mobile. A full 13-case browser session returned 13 live OpenRouter Jev results. These checks establish correct rendering and integration for this run; they do not demonstrate that the interface eliminates decision bias. The case order and wording remain fixed.
